...
export default function Home() {
  ...
  const [cuentaContableTexto, setCuentaContableTexto] = useState('');
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-8 lg:px-10">
        ...
        <section className="mb-5 rounded-2xl border border-[#e1e2e4] bg-white p-5 shadow-[0_8px_28px_rgba(31,52,69,.05)]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1.35fr_auto]">
            ...
            <label className="block"><span className="mb-1.5 block text-xs font-semibold text-[#49505b]">Cuenta contable</span><input type="text" list="cuentas-contables" value={cuentaContableTexto} onChange={(event) => { const text = event.target.value; const normalized = text.trim().toLocaleLowerCase('es'); const selected = cuentasDestino.find((cuenta) => cuenta.nombre.toLocaleLowerCase('es') === normalized || cuenta.codigo.toLocaleLowerCase('es') === normalized); setCuentaContableTexto(text); setCuentaContable(selected?.codigo ?? ''); }} disabled={!cuentasDestino.length} placeholder={cuentasDestino.length ? 'Todas las cuentas · escribí para buscar' : 'Cargando cuentas…'} autoComplete="off" className="w-full rounded-lg border border-[#cdcfd2] bg-white px-3 py-2.5 text-sm text-[#04102d] outline-none transition placeholder:text-[#898e95] focus:border-[#3985ff] focus:ring-2 focus:ring-[#3985ff]/15 disabled:bg-[#f8f8f9] disabled:text-[#898e95]"/><datalist id="cuentas-contables">{cuentasDestino.map((cuenta) => <option key={cuenta.codigo} value={cuenta.nombre}>{cuenta.codigo}</option>)}</datalist>{cuentaContableTexto && !cuentaContable && <span className="mt-1 block text-[11px] text-[#a36b00]">Seleccioná una cuenta de las sugerencias.</span>}</label>
            <button type="button" onClick={() => setRefreshCounter((value) => value + 1)} disabled={apiStatus === 'loading' || !estadoBancario || Boolean(cuentaContableTexto && !cuentaContable)} className="self-end rounded-lg bg-[#3985ff] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(57,133,255,.2)] transition hover:bg-[#017ce2] disabled:cursor-not-allowed disabled:opacity-60">{apiStatus === 'loading' ? 'Actualizando…' : 'Actualizar'}</button>
          </div>
        </section>
