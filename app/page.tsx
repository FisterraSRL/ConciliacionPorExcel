...
export default function Home() {
  ...
  return (
    <main className="finnegans-shell min-h-screen bg-white text-[#1b2432]">
      <header className="finnegans-topbar"><div className="flex w-full items-center gap-5 px-4">
        <button type="button" aria-label="Abrir menú" className="menu-trigger">☰</button>
        <div className="finnegans-brand"><span className="brand-chevron">›</span><strong>Finnegans</strong><span>GO</span></div>
        <label className="top-search"><span>⌕</span><input aria-label="Buscar en Finnegans" placeholder="Buscar..." /></label>
        <div className="top-actions"><span className="top-icon">☆</span><span className="top-icon">♢</span><div className="company-context"><strong>{empresa || 'Empresa'}</strong><small>Empresa</small></div><div className="workspace-context"><strong>Conciliación</strong><small>Espacio de trabajo</small></div></div>
      </div></header>

      <div className="finnegans-content mx-auto max-w-[1800px] px-5 py-7 lg:px-9">
        <div className="page-heading mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-[21px] font-medium text-[#3f4650]">Conciliación por Excel - Situación de Cheques</h2><p className="mt-1 text-sm text-[#747b84]">Importación y conciliación de documentos físicos</p></div><div className={`rounded-sm border px-3 py-1.5 text-xs font-medium ${apiStatus === 'ready' ? 'border-[#aee9d0] bg-[#effcf7] text-[#169568]' : apiStatus === 'error' ? 'border-[#f1c6c6] bg-[#fff4f4] text-[#d34a4a]' : 'border-[#b9dfff] bg-[#f1f8ff] text-[#168df5]'}`}>{apiStatus === 'ready' ? `Finnegans listo · ${apiCount.toLocaleString('es-AR')} cheques` : apiStatus === 'error' ? 'Finnegans no disponible' : 'Consultando Finnegans…'}</div></div>
        ...
        {!rows.length ? <section className="finnegans-upload rounded-2xl border border-[#dfe4e8] bg-white p-4 shadow-[0_8px_28px_rgba(31,52,69,.06)] sm:p-7">
          ...
          <section className="finnegans-grid overflow-hidden rounded-2xl border border-[#dce3e8] bg-white shadow-[0_8px_28px_rgba(31,52,69,.06)]">
