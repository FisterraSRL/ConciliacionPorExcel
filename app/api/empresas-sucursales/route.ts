...
export async function GET() {
  try {
    ...
    const params = new URLSearchParams({ '[updatedSince]': '1900-01-01', ACCESS_TOKEN: token });
    let response: Response | null = null;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      response = await fetch(`${reportBaseUrl}/empresaSucursal/list?${params}`, { cache: 'no-store' });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Error al consultar empresas y sucursales.' }, { status: 502 });
  }
}
